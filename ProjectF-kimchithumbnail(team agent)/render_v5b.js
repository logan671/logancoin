...
const thumbnails = [
  {
    ...
    yes: 52, no: 48,
    bgType: 'image', bgImage: 'kospi_chart_0.jpg', bgBase: '#0a1628',
  ...
  },
  {
    ...
    yes: 100, no: 0,
    bgType: 'image', bgImage: 'culinary_logo_0.png', bgBase: '#0a0a0a', bgContain: true,
  ...
  },
  {
    ...
    extraCSS: `.grid{position:absolute;inset:0;background-image:linear-gradient(rgba(168,85,247,0.04) 1px,transparent 1px),linear-gradient(90deg,rgba(168,85,247,0.04) 1px,transparent 1px);background-size:30px 30px}`,
  ...
  },
  {
    ...
    yes: 68, no: 32,
    bgType: 'image', bgImage: 'hottest_earth_alt_clean.jpeg', bgBase: '#1a0000',
  ...
  },
  {
    ...
    extraCSS: `.dots{position:absolute;inset:0;background-image:radial-gradient(rgba(6,182,212,0.08) 1px,transparent 1px);background-size:16px 16px}`,
    extraBgHTML: '<div class="dots"></div>',
  },
];

// ── HTML 빌더 (동일 크기 버튼 + 밝기 차이) ──

function buildHTML(t, colors) {
  const yesWin = t.yes >= t.no;
  const btnW = 105, btnH = 58; // 동일 크기
  const yc = colors.yes, nc = colors.no;
  const fs = fontSize(t.title);

  // 우세한 쪽: 높은 opacity + 진한 border → 열세한 쪽: 낮은 opacity + 연한 border
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .vote-pct{font-size:13px;font-weight:600;margin-top:2px}
...
async function renderAll() {
  ...
  for (const t of thumbnails) {
    ...
    console.log(`${t.name}: YES(${colors.yes.r},${colors.yes.g},${colors.yes.b}) NO(${colors.no.r},${colors.no.g},${colors.no.b}) | ${t.yes > t.no ? 'YES' : 'NO'} 우세`);
